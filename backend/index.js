...
// app.get("/linkbbp", cors(), async (req, res) => {
//   try {
//     const alldata = await pool.query(
//       "SELECT * FROM public.link_bbp_test"
//     );
//     res.json(alldata);
//     linkbbpdata = alldata.rows;
//     console.log("link bbp DATA API");
//   } catch (error) {
//     console.log(error, "error");
//   }
// });
...
app.post("/selected", cors(), (req, res) => {
  try {
    ...
    const data_doc = pool.query(
      'SELECT * FROM doc_new'
    ).then((data_doc)=>{

      linkbbpdata = data_doc;
      console.log(linkbbpdata, "allData");
    });
  
   let selected_shape = linkbbpdata.filter((e)=> e.shape === 'PEAR')
   
    console.log("selected_shape : ",selected_shape)
    res.send(selected_shape);
